 ...
 */
public class AlbumHomeMultipleRecycleAdapter extends BaseMultiItemQuickAdapter<HomeIndex.ItemInfoListBean, BaseViewHolder> implements BaseQuickAdapter.SpanSizeLookup, BaseQuickAdapter.OnItemChildClickListener {
    ...
    public AlbumHomeMultipleRecycleAdapter() {
        setSpanSizeLookup(this);
        addItemType(ConstantGoods.TYPE_TOP_BANNER, R.layout.jd_homerecycle_item_top_banner);
        addItemType(ConstantGoods.TYPE_ICON_LIST, R.layout.jd_homerecycle_item_icon_list);
        addItemType(ConstantGoods.TYPE_NEW_USER, R.layout.jd_homerecycle_item_new_user);
        addItemType(ConstantGoods.TYPE_JD_BULLETIN, R.layout.jd_homerecycle_item_jd_bulletin);
        addItemType(ConstantGoods.TYPE_JD_SPIKE_HEADER, R.layout.jd_homerecycle_item_spike_header);
        addItemType(ConstantGoods.TYPE_JD_SPIKE_CONTENT, R.layout.jd_homerecycle_item_spike_content);
        addItemType(ConstantGoods.TYPE_SHOW_EVENT_3, R.layout.jd_homerecycle_item_show_event_3);
        addItemType(ConstantGoods.TYPE_FIND_GOOD_STUFF, R.layout.jd_homerecycle_item_find_good_stuff);
        addItemType(ConstantGoods.TYPE_WIDTH_PROPORTION_211, R.layout.jd_homerecycle_item_type_211);
        addItemType(ConstantGoods.TYPE_TITLE, R.layout.jd_homerecycle_item_type_title);
        addItemType(ConstantGoods.TYPE_WIDTH_PROPORTION_22, R.layout.jd_homerecycle_item_type_22);
        addItemType(ConstantGoods.TYPE_WIDTH_PROPORTION_1111, R.layout.jd_homerecycle_item_type_1111);
        addItemType(ConstantGoods.TYPE_MIDDLE_BANNER, R.layout.jd_homerecycle_item_middle_banner);
        addItemType(ConstantGoods.TYPE_SHOW_EVENT_FILL_UP, R.layout.jd_homerecycle_item_show_event_1);
        addItemType(ConstantGoods.TYPE_FIND_GOOD_SHOP, R.layout.jd_homerecycle_item_type_find_good_shop);
        addItemType(ConstantGoods.TYPE_PREFERRED_LIST, R.layout.jd_homerecycle_item_type_perferred_list);
        addItemType(ConstantGoods.TYPE_LIVE, R.layout.jd_homerecycle_item_type_live);
        addItemType(ConstantGoods.TYPE_RECOMMENDED_WARE, R.layout.jd_homerecycle_item_type_recommented_ware);
    }
    ...
    private void bindTypeMiddleBannerData(BaseViewHolder helper, final HomeIndex.ItemInfoListBean item, int position) {
        ...
        banner.setData(R.layout.jd_homerecycle_middle_banner_content, item.itemContentList, null);
    }
    ...
    private void bindJDSpikeContentData(BaseViewHolder helper, HomeIndex.ItemInfoListBean item, int position) {
        ...
        SpikeContentAdapter adapter = new SpikeContentAdapter(R.layout.jd_homerecycle_item_spike_content, item.itemContentList);
    ...
    private void bindJDBulletinData(BaseViewHolder helper, final HomeIndex.ItemInfoListBean item, int position) {
        ...
        viewSwitcher.setContentLayout(R.layout.jd_switch_view);
    }
    ...
    /**
     * 这里导航列表，使用的是拼接布局
     *   比较好的做法是Recyclerview 九宫格布局。
     *   这里不建议这样写，造成监听事件及其他资源的大量浪费和代码冗余
     * @param helper
     * @param item
     * @param position
     */
    private void bindIconListData(BaseViewHolder helper, final HomeIndex.ItemInfoListBean item, final int position) {
        ImageView iconListIcon = (ImageView) helper.getView(R.id.icon_list_one);
    ...
    private void bindTopBannerData(BaseViewHolder helper, final HomeIndex.ItemInfoListBean item, int position) {
        ...
        banner.setData(R.layout.jd_homerecycle_top_banner_content, item.itemContentList, null);
    }
